...
public class TestHttpExecutor {
	...
	public void testPost_external_body_file() throws Exception {
		...
		HttpRecord record = new HttpExecutor().exec(httpRequestItem.getRequest());
		assertEquals(200, record.getResponse().getCode().intValue());
		assertTrue("Response doesn't contain \"data\":\"<body>BodyFromFile1</body>\", responseBody=" + record.getResponse().getResponseBody(),
	...
	public void testPost_external_body_file_with_template() throws Exception {
		...
		HttpRecord record = new HttpExecutor().exec(httpRequestItem.getRequest());
		assertEquals(200, record.getResponse().getCode().intValue());
		assertTrue("Response doesn't contain \"data\":\"<body>BodyFromFile2</body>\", responseBody=" + record.getResponse().getResponseBody(),
	...
	private HttpRequestItem unmarshal(String requestFilePath, List<Argument> arguments) throws Exception {
		HttpRequestItem requestItem = JsonMarshallingUtils.unmarshal(readFile(requestFilePath), HttpRequestItem.class, arguments);
