...
public class JPty {
	...
	public static interface JPtyInterface {
		 ...
		 *            process.
		 * @return a value of <tt>0</tt> upon success, or a non-zero value in case of an
		 *         error (see {@link JPty#errno()} for details).
		 */
		int kill(int pid, int sig);

		/**
		 * Sets the window size information for the process with the given FD using the
		 * given {@link WinSize} structure.
		 * 
		 * @param fd
		 ...
		 * @param stat
		 *            the array in which the result code of the process will be stored;
		 * @param options
	...
	static {
		if (Platform.isMac()) {
			m_jpty = new JPtyMacImpl();
		} else if (Platform.isLinux()) {
			m_jpty = new JPtyLinuxImpl();
		} else {
	 ...
	 * @return <code>true</code> if the process with the given process ID is alive,
	...
	/**
	 * Not public as this method <em>replaces</em> the current process and therefore
	...
	/**
	 * Processes the given command + arguments and crafts a valid array of arguments
	 * as expected by {@link #execve(String, String[], String[])}.
	 * 
	 * @param command
